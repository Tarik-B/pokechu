...
from data.data_enums import Region, Game
...
class Pokemon:
    def __init__(self, unique_id: int, names: dict):
        ...
        self._descriptions = dict() # lang str -> dict{ list(Game) -> str }
    ...
    def add_description(self, lang: str, game: Game, description: str):
        if lang not in self._descriptions:
            self._descriptions[lang] = dict()
        # else:
        #     for game in self._descriptions[lang]:
        #         if self._descriptions[game] == description:

        if game in self._descriptions[lang]:
            print(f"overriding description for {self._unique_id}, game = {game.name}")

        self._descriptions[lang][game] = description
